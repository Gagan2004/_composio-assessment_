...
│   └── verification_sample.json # Stratified human review sample for verification audit
└── src/
...
## 📂 Data Directory Breakdown (`data/`)

The `data/` directory contains the raw seed inputs, master verified datasets, and audit samples powering the pipeline and dashboard:

### 1. `data/apps_raw.json` (Raw Input Seeds)
- **Purpose**: Input seed dataset containing 100 customer-requested SaaS and developer platforms across 10 core categories (CRM, Support, Messaging, Marketing, Ecommerce, Data/SEO, Dev Infra, Productivity, Finance, AI & Media).
- **Structure**: Each entry contains `id`, `name`, `category`, `website_hint`, and initial pass assumptions.

### 2. `data/apps_pass2_verified.json` (Master Output & Pipeline Telemetry)
- **Purpose**: Master output generated by `src/verifier.py` after live HTTP scraping, signal trimming, and Gemini 2.5 Flash inference.
- **Key Fields**:
  - `auth_methods`: Extracted authentication paradigms (`OAuth2`, `API Key`, `Bearer Token`, `PAT`, `Basic Auth`).
  - `self_serve_status`: Gating classification (`Self-Serve`, `Paid/Sandbox Only`, `Enterprise Gated`).
  - `api_surface`: Extracted API protocols (`REST API`, `GraphQL`, `gRPC`, `Webhooks`, `SOAP`).
  - `buildability_verdict`: Assessment for Composio toolkits (`Ready Today`, `Workaround Required`, `Blocked / Outreach Needed`).
  - `main_blocker`: Specific friction or gating rationale (e.g. enterprise sales wall, app review process).
  - `evidence_url` & `live_url_status`: Official documentation link and HTTP response status (`200 OK`).
  - `telemetry`: Pipeline performance metrics (`scrape_duration_s`, `schema_compliance_rate`, `gemini_llm_inference_rate`).

### 3. `data/verification_sample.json` (Audit Sample)
- **Purpose**: Stratified sample dataset powering the **Verification & Accuracy Audit** view in the interactive dashboard.
- **Role**: Allows reviewers to inspect ground-truth accuracy checks and signal verification across representative apps from all 10 categories.
